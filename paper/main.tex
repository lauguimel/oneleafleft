...
The \emph{training set} covers prediction years 2020--2022
(with features computed from 2016 onward) and contains
$N_\mathrm{train}=729\,114$ pixels, of which
$n^+_\mathrm{train}=2\,525$ experienced deforestation
(prevalence 0.35\%), where $n^+$ denotes the number of
positive (deforested) samples.
The \emph{validation set} targets prediction year 2024
(features from 2017 onward; $N_\mathrm{val}=240\,230$,
$n^+_\mathrm{val}=871$, prevalence 0.36\%).
By construction, the training set never includes any observation
from 2023, which is reserved exclusively as lag-1 input
for the 2024 validation predictions.

\paragraph{Classifier.}
We train an XGBoost gradient-boosted tree
ensemble~\cite{Chen2016} with early stopping on validation
log-loss.
To handle the severe class imbalance, we set the
\texttt{scale\_pos\_weight} parameter to the ratio of negative
to positive training samples ($\approx$288), which up-weights
the minority class proportionally.
Hyperparameters are tuned via Bayesian optimisation
(50 trials, Optuna~\cite{Akiba2019}) minimising validation
log-loss; the best configuration is then retrained on the full
training set.
All generalisation claims rest on the held-out 2024 forecast;
no spatial or temporal cross-validation is used during training.
...
We use two complementary ranking metrics.
The \emph{area under the receiver operating characteristic curve}
(AUC-ROC) measures a classifier's ability to rank positive
examples above negative ones across all decision thresholds:
a random classifier scores 0.5, a perfect classifier scores 1.0.
Because AUC-ROC can appear misleadingly high when positives are
rare~\cite{Mayfield2017}, we also report the \emph{area under
the precision--recall curve} (PR-AUC), which focuses on the
model's ability to maintain high precision as recall increases.
For PR-AUC the baseline of a random classifier equals the class
prevalence---here approximately 0.004---so any value substantially
above this threshold indicates genuine predictive skill.

We complement these with two operationally interpretable quantities.
The \emph{concentration curve} $C(s)$ gives the fraction
of total deforestation events recovered when screening the
top-$s$ fraction of pixels ranked by predicted risk;
it measures how tightly the model concentrates risk into a
small target area.
The \emph{lift} at screening fraction $s$ is defined as
$\mathcal{L}(s) = \mathrm{recall}(s)/s$;
a lift of 23 at $s=0.01$ means the top 1\% of predicted pixels
contains 23 times more deforestation events than a random
selection of 1\% of pixels would.
...
Feature ablation results on the 2024 validation set.
...
(b)~Spatial ablation: PR-AUC for seven buffer-radius configurations;
AUC-ROC annotated on each bar.}
\end{figure}

\subsection{Spatial ablation: multi-scale buffers improve minority recall}

Figure~\ref{fig:ablation}(b) and Table~\ref{tab:spatial} compare
...
\begin{table}[htbp]
\begin{center}
\caption{\label{tab:temporal}
Temporal ablation: validation metrics as a function of
lag-window depth (core feature set).}
\begin{tabular}{lccc}
\toprule
Window depth & Features & AUC-ROC & PR-AUC \\
\midrule
1 year  &  29 & 0.954 & 0.076 \\
2 years & 104 & 0.957 & 0.075 \\
3 years & 119 & 0.955 & 0.077 \\
4 years & 139 & 0.957 & 0.076 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

Table~\ref{tab:temporal} shows that extending the temporal window
from one to four lag years yields negligible improvement in both
metrics ($\Delta$AUC\,$<$\,0.003, $\Delta$PR-AUC\,$<$\,0.002),
with the single-year model already achieving AUC-ROC 0.954 and
PR-AUC 0.076.
...
High-resolution deforestation map and 2025 forecast.
(a)~Cumulative deforestation status by 2024 at 250\,m spacing
(14{,}641~points): green\,=\,forested, violet\,=\,deforested
(Hansen GFC\,v1.12 \texttt{lossyear}\,$\in$\,[1,24]).
(b)~Predicted deforestation risk for 2025 (same grid).
Green-to-red gradient indicates predicted risk for currently forested pixels;
violet pixels are already deforested (carried from panel~a).
